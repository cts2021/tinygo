...
var (
	// Heap allocations during interp evaluation. These are stored as global
	// variables.
	allocSymbolRegexp = regexp.MustCompile(`\$alloc(\.[0-9]+)?$`)

	// String constants (the buffer behind strings). Stored as global arrays.
	stringSymbolRegexp = regexp.MustCompile(`\$string(\.[0-9]+)?$`)
)
...
func loadProgramSize(path string, packagePathMap map[string]string) (*programSize, error) {
	...
	var data *dwarf.Data
	elfMachine := elf.EM_NONE
	var sections []memorySection
	var codeOffset uint64 // offset added to code addresses (for wasm)
	if file, err := elf.NewFile(f); err == nil {
		...
		for _, symbol := range allSymbols {
			...
			if allocSymbolRegexp.MatchString(symbol.Name) || stringSymbolRegexp.MatchString(symbol.Name) {
		...
		codeOffset = 0x8000_0000_0000_0000
	...
	for {
		if elfMachine == elf.EM_AVR && strings.Split(llvm.Version, ".")[0] <= "10" {
...
func findPackagePath(path string, packagePathMap map[string]string) string {
	...
	if !ok {
		...
		} else if allocSymbolRegexp.MatchString(path) {
			// Parse symbol names like main$alloc.
			packagePath = path[:strings.LastIndex(path, "$alloc")]
		} else if stringSymbolRegexp.MatchString(path) {
			// Parse symbol names like main$string.
			packagePath = path[:strings.LastIndex(path, "$string")]
		} else {
